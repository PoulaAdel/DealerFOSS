...
| Reconditioning attributed to each stock unit and relieved on sale | Accounting | Build | **Found 2026-09-19.** The workshop posts recon to 1300 but uses the owned unit id only as an ownership check. It neither adds recon to the unit's acquisition cost nor stamps the unit id on that posting. Delivery still uses the unit's recorded cost. A per-car carrying value needs durable attribution to that stay in stock and correction/sale handling; the stock screen must not call acquisition cost a recon-inclusive book value |
