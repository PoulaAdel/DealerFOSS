...
**First, the line this section is not about.** A deal has recorded its own finance
structure since 2026-09-25 — a provider's name, the cash down, an APR and a term,
with the monthly payment, the total of payments and the finance charge worked out
from them. None of that needs a credit report, a lender or a contract, because a
payment is arithmetic over an amount financed, a rate and a term. What needs
everything below is **submitting an application and receiving a decision**:
pulling a consumer's credit file, sending it anywhere, and acting on what comes
back. Recording what was agreed and asking a lender to agree to it are two
different capabilities, and only the second one is blocked.
...
| **D6** | How much cost to relieve when an F&I product is cancelled | Raised 2026-09-21 by a walk that found the department reading a **−$460 loss** where it made +$240. The revenue is reversed and the cost is not, so every cancellation makes F&I look worse than it was, permanently. This cannot be fixed by guessing: a cancellation inside the free-look period returns the whole of what the dealership paid the underwriter, a later one returns it pro-rata over the unexpired term, and some providers keep a fee. The refund handed to the customer does not say which happened. **Recommendation: relieve the cost in the same proportion as the price refunded** (`refund ÷ price × cost`), which is exactly right for a full cancellation and the closest defensible figure for a partial one, and let the person cancelling state a different figure when the provider's remittance says otherwise — the same escape hatch, and for the same reason, as person-entered tax. Until it is decided, the ledger and `Deal.ProductCost` disagree and the ledger is the one reports read |

**Three left.** D1 is a market decision and belongs to the business; D5 and D6
are ours. Everything else on this list has been settled and written down.
...
| A sync run that starts without a person | Integration | Done | **Built 2026-09-26.** `ConnectorSchedule` stores the standing instruction and `ScheduleWorker` claims the earliest due feed; before this the runtime had no caller anywhere in `src`. The polling is unattended, the run carries the authority of whoever armed it, and **the grant is re-read on every fire**, so a feed suspends rather than writing on a permission that no longer exists ([ADR-028](adr/0028-a-scheduled-run-carries-the-authority-that-armed-it.md)). A poll on a clock, not a push: no webhook, no inbox, no lease beyond a conditional update on one row, no cron. No screen yet |
...
| Reconditioning capitalised onto the car | Accounting | Done | **Rechecked 2026-09-19.** Internal work on owned stock debits 1300 instead of 5400. This is ledger capitalisation only: it does not update the individual unit's cost. Per-car attribution and relief on sale are tracked below |
...
| Technician clock: flagged against clocked hours | Service | Done | **Built 2026-09-16**, hours after the op codes that made it possible. Clock on and off a job; clocking on elsewhere closes the open one and records why, because a shop that had to clock off first would stop clocking. Several technicians on one job is ordinary, so the one-open rule is per technician and a partial unique index enforces it. Productivity came off the labour report's "cannot measure" list; **Efficiency stays there**, because hours produced over hours *available* needs a roster, and a roster is payroll |
...
| Year-end close, and comparatives against last year | Reporting | Done | **Built 2026-09-23, with a screen.** Closing a year is a posting like any other, the same discipline opening balances already follow: every revenue and expense account's balance for the year is zeroed by one ordinary `JournalEntry.Post`, with the net carried to the new RetainedEarnings (3100) account — `BalanceSheetAsync`'s all-time earnings formula needed no code change, because the account balances themselves are what changed. Every month in the year must already be closed. A closed year refuses postings the same way a closed month does, checked independently in `PeriodRefusalAsync`, and reopening stays possible and stays on the record. The P&L now carries a `PriorYear` field, the same report shifted back one year, rendered as a column beside this year's. Cash flow remains out of scope |
...
| Receivable ageing, statements and credit limits | Accounting | Done | **Built 2026-09-17, with a screen.** Ageing buckets outstanding bills by 30/60/90-day bands from the existing DaysOutstanding figure; a statement slices a customer's whole receivable history to a period with a running balance computed server-side; a nullable credit limit on Customer is checked in ReceivableService.OpenAsync before a deal or repair order opens new debt, same-currency only |
...
| Tax as a line on the deal summary | Deals | Done | **Built 2026-09-19.** Tax is in `AmountDue` on the server, so it is now in the column that adds up to it, with the basis and rate on the row. This was the THIRD time this table lost a line — after the trade-in and the products — so the fix is a test that reads every amount in the column and asserts it reaches the printed total, rather than a fourth one-off. Walked: a seeded deal reads down to $13,638.15 against a printed $13,638.15 |
| A documentation fee a person can add | Deals | Done | **Built 2026-09-19.** `ChargeKind.DocumentationFee` has existed in the domain and the ledger since 2026-09-09, as its own kind because tax treats it differently from a registration fee (ADR-024); `contracts.ts` and the six locales learned it today, so the one charge a dealership adds to nearly every deal can be entered on a screen |
...
| A URL for every record | Platform | Done | **Done 2026-09-17.** `/customers/:id`, `/leads/:id`, `/deals/:id`, `/inventory/:id` and `/workshop/:id`, as an OPTIONAL segment on the same route so the list is never remounted. The detail band is unchanged, per the 2026-09-16 amendment to ADR-020. Signing in keeps the address, so a shared link survives a lapsed session |
...
| Pickers that work at dealership scale | Platform | Done | **Built 2026-09-15.** One `RecordPicker` searching the server, on the booking screen, the enquiry form and the deal desk. Every option carries what tells two identical records apart — a VIN tail, a stock number, an email — because the labels were the defect, not the length of the list. Booking narrows the cars to the ones that customer has been here with, derived from the workshop's own records since a vehicle has no owner. The enquiry form stopped offering **sold** cars, which it had done since it was written. Parts is deliberately untouched |
| Stock valued, not just counted | Reporting | Done | **Built 2026-09-19.** Acquisition cost and currency now travel with every scoped stock-list row, visible without opening each car. Missing cost reads "not recorded" and a recorded zero stays zero. List and detail use the same figure and label in all six languages. This closes the per-car acquisition-cost display gap; recon-inclusive carrying value is a separate gap discovered while verifying the source |
| Reconditioning attributed to each stock unit and relieved on sale | Accounting | Done | **Found and fixed 2026-09-19.** A `ReconditioningCharge` row per capitalised posting names the car, the amount and the repair order behind it, so a car's book value is acquisition plus everything spent making it saleable. Delivery relieves the book value, so 1300 comes back to where it started — before this it was short by the recon on every reconditioned car, forever, and used-vehicle gross was overstated by the same amount. Append-only, so a correction is a negative row rather than an edit. The stock detail shows the three figures separately; the list column still says "Acquisition cost", which it is |
| An address on a customer, and a registration address on a deal | Customer | Done | **Built 2026-09-19.** `PUT /customers/{id}/address` for the customer's own; `Deal.RegistrationAddress` via `POST /deals/{id}/registration-address` for the fact ADR-024 traces tax to, its own Deals-owned type, frozen once the deal leaves Draft. Both on screen in all six languages |
| Rate x basis worked out on the tax band | Deals | Done | **Built 2026-09-19.** The amount follows basis x rate as either is typed, rounded to the cent. Still overridable, deliberately — `EnteredByPerson` exists so an unsupported jurisdiction is a label rather than a blocker, and a capped or tiered tax is not a multiplication — but an override that contradicts its own basis and rate now says so on the row, with the figure the arithmetic gives. A saved line is never silently rewritten |
| Move one lot to another installation, ids and relationships intact | Integration | Done | **Built 2026-09-21, with a screen.** The last buildable unmet exit criterion. A JSON records package (ADR-027) carries one lot's stock, sales and workshop jobs plus the people and cars they name, keeping every id so a sale still points at the right car on the far side. Documents are re-rendered rather than copied, because this system stores no files — the test renders both documents on both sides and compares them character for character, which is what caught two printed-order row-ordering defects nobody had ever needed to be stable. Walked across two tenant databases: 5 written, 315 already here, 571 refused and every one named |
| The printed vehicle order adds up | Documents | Done | **Found and fixed 2026-09-21**, by reading one in a browser. Tax is inside `AmountDue` and was missing from the column printed above it, so a real order showed $33,000.00 of lines under a $36,331.25 total — the customer's own paperwork short by the tax on every deal that carried any. The FOURTH time a line inside the total has been missing from a column here, and the second copy of the same column, so the fix is the same property test the deal desk grew: every amount in the rendered HTML is summed and must reach the printed total. The row now carries the basis and the rate, because a buyer querying a tax figure is asking on what and at what rate |
| A monthly payment on a deal | Finance | Done | **Built 2026-09-25.** A deal had no APR, no cash down, no term and no payment anywhere on it, so the desk could price a car, take a trade, sell cover and work out tax and could not answer the first question every retail buyer asks. `Deal.Financing` stores four figures — provider, cash down, rate, term — and everything else is derived: the amount financed is `AmountDue` less the down payment, and the payment is the standard annuity, with the schedule **walked in decimal** so the balance provably reaches zero rather than multiplied out. On the desk and on the printed order, in all six languages. **It needed no lender**: a payment is arithmetic, so recording what was agreed needs nothing outside the installation — submitting an application and receiving a decision still do and are still absent. The one trap: financing sits **outside** `AmountDue` on purpose, because a down payment is how the customer pays rather than money off, so the payment terms print below the total and a test asserts the column above still reaches it. Carried in the records package too — it is the one component whose loss the package's amount-due check cannot see |
| F&I cost is never relieved when a product is cancelled | Finance | Build | **Found 2026-09-21, not fixed — needs a decision first.** `PostProductCancellationAsync` reverses the revenue and raises the customer credit and touches nothing on the cost side, so the underwriter's charge stays on the books as if the policy were still in force. September reads F&I revenue $500, cost $960, **gross −$460**, where the one product still in force made +$240. `Deal.ProductCost` already excludes cancelled products, so the deal screen and the ledger disagree with each other. The open question is how much cost to relieve: a full refund inside the free-look period returns the whole cost, a later cancellation is pro-rata, and the refund amount alone does not say which. Recommendation in §11 |
| Job number that says which rooftop | Service | Done | **Built 2026-09-15, and half the finding was wrong.** The printed job card was never ambiguous — every document has carried the dealership and the lot since documents existed. Only the screen was short of it. The lot now shows beside the number when the list actually mixes lots, and always on an open job. The stored numbers are untouched: a rooftop prefix would rewrite job cards customers already hold, which is a decision rather than a detail |
| Security testing of what is built | Platform | Done | **Done 2026-09-24, and it found one real defect.** An adversarial pass, written up in [`implementation/SECURITY-TESTING.md`](implementation/SECURITY-TESTING.md) as attacks tried rather than controls that exist. `/receivables/for/{source}/{reference}` answered 204 for a reference nobody has and 403 for a bill at a lot the caller cannot see, so the status code alone confirmed a deal or job had been invoiced somewhere in the group — fixed, and the reach is still audited. Everything else held: eight by-id routes and both printed documents refused across the rooftop boundary and refused *identically* to a record that does not exist, seven list filters could not be widened by naming a lot, a write naming another lot in its body was refused, both halves of the nine-day-old records-package endpoints were refused without the group-wide right, stored markup printed as text, and there is no raw SQL in `src` at all. Twelve credential routes are now asserted to carry the limiter, and a sweep fails on any new `/auth` or `/admin` route nobody has classified — it immediately named five that were outside the check. **This is not the independent penetration test doc 06 requires**; that still needs somebody outside the project |
