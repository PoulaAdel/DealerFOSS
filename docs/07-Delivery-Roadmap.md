...
| R10 | compliance rules become unmaintainable | High | versioned jurisdiction/provider packs with owners, evidence tests, and review dates — given a concrete shape by [ADR-024](adr/0024-compliance-is-baseline-pack-and-posture.md): a pack carries data and declarations, never logic, and `Supported` means a person reviewed it |
...
| **Target market** | **overdue — it gates Phase 0** | which market first: franchised or independent (doc 11 **D1**). **The row that used to sit here had lost its name and deadline in an edit and read as a fragment.** Reconstructed 2026-08-14 and promoted. **Halved 2026-09-05:** the *jurisdiction* half is settled by [ADR-024](adr/0024-compliance-is-baseline-pack-and-posture.md) — US first for compliance depth, architecture country-neutral, rules carried as versioned packs. What remains is the market itself, and it still gates the OEM programme, the ambassador role and the EVR/ERT contracts |
| ~~Jurisdiction~~ | ~~Phase 0~~ | **Closed 2026-09-05 by [ADR-024](adr/0024-compliance-is-baseline-pack-and-posture.md).** This row used to argue that "§4 scopes US-only compliance while the product ships six languages with full RTL — both cannot be the priority". **The tension was overstated.** Shipping six languages is already done and carries no ongoing compliance cost; a language is not a jurisdiction. Compliance is the singular thing, the product is not |
