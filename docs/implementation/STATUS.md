...
- **2026-09-25 — A deal can say what the customer pays a month.** Checked by name across `src/App/Deals` and `src/App/Finance` first: there was **no APR, no cash down, no amount financed, no term and no monthly payment anywhere on a deal**. `src/App/Finance` is the F&I product catalogue and nothing else, and the only `TermMonths` in the deal contracts belonged to a warranty. So the desk could price a car, take a trade, sell cover and work out tax, and could not answer the first question every retail buyer asks — the number the whole department is built around.

  **It needed no lender, and that line is worth stating because it is why this was buildable at all.** A payment is arithmetic over an amount financed, a rate and a term, so recording what was agreed needs nothing outside the installation. Submitting a credit application and receiving a decision do, and remain absent — the provider here is a name the dealership typed, and nothing is sent anywhere. `docs/11` §4 now opens by drawing that line, because the whole section reads as though every part of finance were contract-blocked.

  **Four figures are stored and everything else is derived.** `Deal.Financing` holds a provider name, the cash down, an APR and a term. The amount financed is `AmountDue` less the down payment; the payment is the standard annuity, `P·i·(1+i)^n / ((1+i)^n − 1)`, and `P/n` at nothing percent, which is a real manufacturer offer rather than a division to guard against. Storing the amount financed or the payment would give the deal two answers to one question and they would disagree within a month — the same rule as `DealProduct.Gross`, and the reason there is no fifth column.

  **The schedule is walked, not multiplied, and that is the part that makes the figures trustworthy.** A rounded payment times the term is not what a customer pays: the last instalment clears the balance. `PlanFor` walks the months in `decimal` — no `Math.Pow`, so no binary approximation at the root of every payment in the system, the power being computed by repeated decimal multiplication over a term capped at 120 — and every derived figure comes out of that one loop, so the plan cannot claim a total the payments do not reach. `The_schedule_pays_the_loan_off_exactly` re-amortises the plan independently, in the test rather than in the code under test, and asserts the balance lands on **zero**. The worked example is a textbook one with a published answer: $20,000 at 6% over 60 months is **$386.66**, checkable against any amortisation table rather than against this file.

  **The trap, and the reason it is written into four separate comments.** Financing sits **outside** `AmountDue` on purpose: a down payment is how the customer pays, not a reduction in what they owe. The receivable opens at the full amount due and the down payment settles part of it like any other receipt, so netting it would make the same money disappear twice — and it would break both "read the column down and reach the total" tests at once. The payment terms therefore print **below** the total, on the desk and on the order, and on the document they carry a `figure` class rather than `num` because `num` is how `ColumnOf` decides what belongs in the sum. Five lines have gone *missing* from a summary column in this product; `The_payment_terms_stay_out_of_the_column_that_adds_up` is the first guard against one being **added** to it that does not belong.

  **A defect found by the change, in somebody else's test, and fixed at the cause.** Adding financing to the records-package fixture made `A_deal_that_does_not_come_to_what_it_says_is_refused` fail: it clears a packaged deal's charges and expects `deals.total_disagrees`, and got `deals.invalid` instead, because the now-tiny total made the down payment illegal and `SetFinancing` threw first. The down-payment check therefore **shadowed the one guard that makes a package trustworthy**, answering "that down payment covers the whole total" where the truth was "this deal says 20,000 and its lines come to 251" — which sends somebody looking at the finance terms for a fault in the charges. `Deal.Import` now assigns the structure rather than setting it: an arriving deal is a record of what another system settled, not a deal being worked, and the total check is the authority on its arithmetic. `Financing.Create` still validates the rate, the term and the sign; only the comparison against a total is left out, and a record whose financing genuinely does not fit arrives showing no payment, which is visible.

  **Carried in the records package**, which is not optional: financing is the one component whose loss the package's amount-due check **cannot** see, precisely because it sits outside the total. A package that dropped it would balance perfectly and still arrive having turned a sixty-month contract into a cash deal. The `PackageTests` fixture is now financed, so the character-for-character paperwork comparison covers it, with the provider named explicitly as well.

  **Refused rather than charged:** a rate at or above 1, because `6.49` typed for `0.0649` is the one data-entry slip this field will really see and a 649% loan produces a number that looks considered; a term outside 1–120; a negative down payment; a down payment covering the whole total, which is a cash deal rather than a structure. The screen's box asks for a **percentage** and converts once on save, because the rate sheet says 6.49 and the contract carries 0.0649, like a tax rate at the same `decimal(9,6)`.

  **Frozen with the rest of the numbers**, for a reason of its own rather than for consistency: the payment derives from `AmountDue`, which freezes on submission, so financing that could move afterwards would requote the customer behind the manager's approval. The order-of-operations hazard is handled both ways — a reprice that leaves nothing to finance returns a **null** plan rather than throwing, because a computed property that threw would turn a half-worked draft into a failed request, and `EnsureReadyToSubmit` refuses it at the gate so no manager receives a financed deal with no payment on it.

  **Walked in a browser** at `F16621F5B`, a $20,900 deal financed at 6.49% over 60 months with $3,000 down: amount financed **$17,900.00**, monthly **$350.15**, final **$350.18**, total **$21,009.03**, finance charge **$3,109.03** — every figure matching an independent calculation, and the balance closing to zero. The deal's own column still read $20,000 + $900 = **$20,900** with the financing outside it, and the printed order's `num` cells summed to the printed total while all nine finance figures sat in `figure` cells below it. Read again in **Arabic**: `dir="rtl"`, the plural correctly "شهرًا" for 60, the provider name untranslated as dealership data, and no horizontal overflow at 375px — measured by scrolling, not by `scrollWidth`.
