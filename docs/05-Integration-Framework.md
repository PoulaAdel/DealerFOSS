...
Step 1 is now half-built and step 2 is untouched. **A run can start with nobody at a keyboard** — `ConnectorSchedule` stores a dealership's standing instruction to read one feed, and `ScheduleWorker` claims the earliest due one. That closed stage 2's last named gap on 2026-09-26. Until then this paragraph said "the only thing that can start a run today is a caller holding a connector instance", and it was accurate: the runtime had no caller anywhere in `src`.

What is still absent, named rather than implied: **no webhook path and no durable inbox**, so nothing pushes and step 2 has nowhere to store a raw envelope before acknowledging. **No poll lease** in the sense meant above — the claim on a due schedule is a conditional update on one row, which stops two application instances firing the same feed and says nothing about a provider's per-tenant limits. **No cron and no backoff**: a schedule is an interval with a five-minute floor. And every capability other than Customers still reports a run as misconfigured, so a schedule armed against one suspends on its first tick. `src/App/Integrations/README.md` keeps the current list, and is the one to correct first when this changes.

**Whose authority a scheduled run carries is [ADR-028](adr/0028-a-scheduled-run-carries-the-authority-that-armed-it.md)**, and it is the decision that shapes the trigger. The polling is unattended; the run is not. A schedule names the person who armed it, the run is made in their scope through the owning capability's `IRecordSink`, and **the grant is re-read every time it fires** — so a feed cannot outlive the permission that created it, and one whose owner has lost that permission suspends with a reason instead of writing on a grant that no longer exists.
